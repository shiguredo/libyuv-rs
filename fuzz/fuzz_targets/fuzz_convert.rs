...
fuzz_target!(|input: FuzzImage| {
    ...
    // I420 → ARGB / ABGR / RGB24 / NV12 / NV21 / I422 / I444
    {
        ...
        let mut dst = AbgrImageMut { data: &mut dst_abgr, stride: width * 4 };
        ...
        let mut dst_uv = vec![0u8; chroma_len];
        let mut dst = Nv12ImageMut {
            y: &mut dst_y, y_stride: width,
            uv: &mut dst_uv, uv_stride: width,
        };
    ...
    // ARGB → I420 / ABGR / RGB24 / NV12 / NV21 / I422 / I444
    {
        ...
        let mut dst = ArgbImageMut { data: &mut dst_argb2, stride: width * 4 };
        let _ = argb_copy(&src, &mut dst, size);
    }

    // ABGR → I420 / ARGB
    {
        let src = AbgrImage { data: &argb, stride: width * 4 };
    ...
    {
        let src = Nv21Image { y: &y, y_stride: width, uv: &chroma, uv_stride: width };
