...
// シンボル書き換え用のプレフィックス
//
// prebuilt で配布する際、他のライブラリが同じ libyuv シンボルを
// 使っていると衝突する。この定数のプレフィックスを付けることで回避する。
//
// libyuv の公開シンボルには opus_ のような統一プレフィックスがないため
// (I420ToNV12, ARGBToI420 等)、全シンボルに単純にプレフィックスを付与する。
//
// 変換例:
//   I420ToNV12  → shiguredo_yuv_I420ToNV12
//   ARGBToI420  → shiguredo_yuv_ARGBToI420
const SYMBOL_PREFIX: &str = "shiguredo_yuv";
...
fn build_from_source(out_dir: &Path, output_bindings_path: &Path) -> PathBuf {
    ...
    let callbacks = rewrite_symbols(&dst.join("lib"), out_dir);
...
// 指示するため、プラットフォーム固有のシンボル名（macOS なら _shiguredo_yuv_I420ToNV12）を
// そのまま返す必要がある。
...
fn rewrite_symbols(lib_dir: &Path, out_dir: &Path) -> SymbolLinkNameCallbacks {
    ...
    //
    // libyuv の公開シンボルには opus_ のような統一プレフィックスがないため、
    // 全シンボルに単純に SYMBOL_PREFIX_ を付与する。
    //   例: I420ToNV12 → shiguredo_yuv_I420ToNV12
    let rename_symbol = |name: &str| -> Option<String> { Some(format!("{SYMBOL_PREFIX}_{name}")) };
...
/// 静的ライブラリのパスを探す
///
/// cmake のビルド結果は Unix 系では libyuv.a、Windows では yuv.lib として出力される。
fn find_static_library(lib_dir: &Path) -> PathBuf {
    let unix_path = lib_dir.join("libyuv.a");
    if unix_path.exists() {
        return unix_path;
    }
    let win_path = lib_dir.join("yuv.lib");
    if win_path.exists() {
        return win_path;
    }
...
///   macOS では _I420ToNV12 → _shiguredo_yuv_I420ToNV12 のようにプラットフォーム固有の
///   `_` プレフィックスを含む形で管理する。
///
/// bindgen_map: Rust バインディングの #[link_name] に使うマップ。
///   キーは C シンボル名 (I420ToNV12)、値はプラットフォーム固有のシンボル名
///   (_shiguredo_yuv_I420ToNV12) を格納する。
...
fn build_symbol_rename_maps(
    ...
    for sym in symbols {
        ...
        if let Some(new_c_name) = rename_symbol(c_name) {
            ...
            //   macOS: shiguredo_yuv_I420ToNV12 → _shiguredo_yuv_I420ToNV12
            //   Linux/Windows: shiguredo_yuv_I420ToNV12 → shiguredo_yuv_I420ToNV12 (変化なし)
